...
# Combinar leads existentes com novos leads
todos_leads_final = leads_sem_site_com_whatsapp + novos_leads

# Ordenar por segmento e cidade
todos_leads_final.sort(key=lambda x: (x.get('segmento', ''), x.get('cidade', '')))
...
wb = load_workbook('leads_sem_site_whatsapp.xlsx')
...
# Limpar conteúdo anterior (manter cabeçalho)
for row in ws_resumo.iter_rows(min_row=2, max_row=10, max_col=4):
    for cell in row:
        cell.value = None
...
for row in ws_leads.iter_rows(min_row=2, max_row=ws_leads.max_row, max_col=9):
    for cell in row:
        cell.value = None

# Preencher com todos os leads
for i, lead in enumerate(todos_leads_final, 1):
    ...
    # Links (usar links pré-definidos se disponíveis)
...
# Atualizar filtro automático
ws_leads.auto_filter.ref = f"A1:I{len(todos_leads_final) + 1}"

# ==========================================
# ATUALIZAR ABA POR SEGMENTO
# ==========================================
ws_segmento = wb['Por Segmento']

# Limpar dados existentes
for row in ws_segmento.iter_rows(min_row=2, max_row=ws_segmento.max_row, max_col=6):
    for cell in row:
        cell.value = None
