...
from cache import get_cache, set_cache, cached_llm_call
...
class GeminiProvider(LLMProvider):
    ...
    async def chat(self, message: str, history: List[Message] = None, use_cache: bool = True) -> str:
        if not self.model:
            return "Gemini nao configurado. Verifique GOOGLE_API_KEY."

        cache_key_str = f"gemini:{hashlib.md5(message.encode()).hexdigest()[:16]}"
        if use_cache:
        ...
        try:
            ...
            if use_cache:
                set_cache(cache_key_str, result, 3600)
...
class OpenAIProvider(LLMProvider):
    ...
    async def chat(self, message: str, history: List[Message] = None, use_cache: bool = True) -> str:
        ...
        cache_key_str = f"openai:{hashlib.md5(message.encode()).hexdigest()[:16]}"
        if use_cache:
            cached = get_cache(cache_key_str)
            if cached:
                return cached
...
class AnthropicProvider(LLMProvider):
    ...
    async def chat(self, message: str, history: List[Message] = None, use_cache: bool = True) -> str:
        ...
        cache_key_str = f"anthropic:{hashlib.md5(message.encode()).hexdigest()[:16]}"
        if use_cache:
            cached = get_cache(cache_key_str)
            if cached:
                return cached
...
class NVIDIAProvider(LLMProvider):
    ...
    async def chat(self, message: str, history: List[Message] = None, model_key: str = None, use_cache: bool = True) -> str:
        if not self.client:
            return "NVIDIA AI nao configurado. Verifique NVIDIA_API_KEY."

        cache_key_str = f"nvidia:{model_key or 'default'}:{hashlib.md5(message.encode()).hexdigest()[:16]}"
        if use_cache:
            cached = get_cache(cache_key_str)
            if cached:
                return cached
    ...
    async def stream_chat(self, message: str, history: List[Message] = None, model_key: str = None):
        if not self.client:
            yield "NVIDIA AI nao configurado. Verifique NVIDIA_API_KEY."
            return
        ...
        try:
            ...
            stream = await self.client.chat.completions.create(
                model=model_id, messages=messages, temperature=0.5, max_tokens=4096, stream=True
            )
            async for chunk in stream:
                if chunk.choices[0].delta.content:
                    yield chunk.choices[0].delta.content
        except Exception as e:
            yield f"Erro na NVIDIA AI: {str(e)}"
...
# =============================================================================
# JOB QUEUE SYSTEM (Background Workers)
# =============================================================================
...
class JobQueue:
    ...
    async def _worker(self, name: str):
        while self._running:
            ...
            try:
                result = await self._process_job(job["type"], job["payload"])
    ...
    async def _process_job(self, job_type: str, payload: Dict) -> Dict:
        if job_type == "site-hunter":
            return await _run_site_hunter(payload)
        elif job_type == "site-analyst":
            return await _run_site_analyst(payload)
        elif job_type == "site-analyst-batch":
            return await _run_site_analyst_batch(payload)
        elif job_type == "site-builder":
            return await _run_site_builder(payload)
        elif job_type == "prospector":
            return await _run_prospector(payload)
        elif job_type == "batch-llm-analyze":
            return await batch_llm_analyze(payload.get("leads", []), payload.get("modelo", "nvidia"), payload.get("batch_size", 10))
        else:
            raise ValueError(f"Job type desconhecido: {job_type}")
...
job_queue = JobQueue(num_workers=3)
...
async def chat_stream(request: ChatRequest):
    async def generate():
        provider = get_provider(request.model)
        try:
            if isinstance(provider, NVIDIAProvider) and hasattr(provider, 'stream_chat'):
                async for chunk in provider.stream_chat(request.message, request.history, model_key=request.model):
                    yield f"data: {json.dumps({'response': chunk, 'model': request.model, 'timestamp': datetime.now().isoformat()})}\n\n"
            else:
                response = await provider.chat(request.message, request.history)
                yield f"data: {json.dumps({'response': response, 'model': request.model, 'timestamp': datetime.now().isoformat()})}\n\n"
        except Exception as e:
...
async def list_nvidia_models():
    return {
        "models": [
            {"id": k, "name": v.split("/")[-1].replace("-", " ").title(), "full_id": v, "speed": "fast" if "8b" in k or "3b" in k else "slow" if "405b" in k or "70b" in k else "medium"}
...
async def run_pipeline(req: PipelineRequest):
    agent = req.agent
    valid_agents = ["site-hunter", "site-analyst", "site-analyst-batch", "site-builder", "prospector"]
...
async def _execute_pipeline(run_id: str, agent: str, params: Dict):
    ...
    try:
        ...
        elif agent == "site-analyst-batch":
            result = await _run_site_analyst_batch(params)
...
async def _analyze_sites_parallel(urls: list[str], modelo: str = "nvidia", concurrency: int = 5) -> list[dict]:
    from scraper_fast import batch_extract_sites

    provider = get_provider(modelo)
    site_data = await batch_extract_sites(urls, concurrency)

    async def analyze_one(data: dict) -> dict:
        if not data or data.get("erro"):
            return {"url": data.get("url", ""), "erro": "Falha ao extrair"}

        prompt = f"""Analise este site e retorne JSON com:
- score_visual (1-10)
- classificacao (feio/ruim/medio/bom/excelente)
- problemas: lista de {{categoria, severidade, descricao}}
- melhorias_sugeridas: lista de strings
...
HTML size: {data.get('html_size', 0)} bytes
...
Retorne APENAS JSON válido."""
        ...
        try:
            ...
            return {"url": data.get("url", ""), **parsed}
        except Exception as e:
            return {"url": data.get("url", ""), "erro": str(e)}

    tasks = [analyze_one(d) for d in site_data]
    return await asyncio.gather(*tasks)


async def _run_site_analyst_batch(params: Dict):
    urls = params.get("urls", [])
    modelo = params.get("modelo", "nvidia")
    concurrency = params.get("concurrency", 5)

    if not urls:
        return {"error": "Lista de URLs vazia"}

    resultados = await _analyze_sites_parallel(urls, modelo, concurrency)
    return {
        "message": f"Analise em lote concluida para {len(urls)} sites",
        "total": len(resultados),
        "resultados": resultados
    }


async def batch_llm_analyze(leads: list[dict], modelo: str = "nvidia", batch_size: int = 10) -> list[dict]:
    provider = get_provider(modelo)
    results = []
    ...
    for i in range(0, len(leads), batch_size):
        ...
        prompt = f"""Analise estes {len(batch)} sites e retorne JSON array com para cada:
- url
- score_visual (1-10)
- classificacao (feio/ruim/medio/bom/excelente)
- problemas: lista de {{categoria, severidade, descricao}}
...
{json.dumps([{{
    ...
    "emails": l.get("emails", []),
    "phones": l.get("phones", []),
}} for l in batch], ensure_ascii=False, indent=2)}

Retorne APENAS JSON array válido."""

        try:
            response = await provider.chat(prompt, use_cache=False)
            parsed = json.loads(response)
            results.extend(parsed)
        except Exception as e:
            for l in batch:
                results.append({"url": l.get("url", ""), "erro": str(e)})
...
async def enqueue_job(req: JobRequest):
    valid_types = ["site-hunter", "site-analyst", "site-analyst-batch", "site-builder", "prospector", "batch-llm-analyze"]
    if req.job_type not in valid_types:
        raise HTTPException(status_code=400, detail=f"Job type invalido. Use: {valid_types}")
    job_id = await job_queue.enqueue(req.job_type, req.payload)
...
async def list_jobs(limit: int = 20):
    return {"jobs": job_queue.list_jobs(limit)}
